...
export default {
    async fetch(request: Request, env: Env): Promise<Response> {
        ...
        try {
            ...
            const existingFiles = await env.RESUME_STORAGE.list({ prefix: `v`, limit: 100 });
            ...
            if (deployResponse.ok) {
                console.log("✅[WORKER]:Deploy hook 호출: Cloudflare Pages Rebuild");
            }
            return new Response(
                JSON.stringify({ success: true, message: `JSON saved as ${versionedKey}` }),
                {
                    headers: {
